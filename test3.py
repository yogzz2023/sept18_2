    ...
    for i, measurement in enumerate(measurements):
        ...
        for track_id in range(len(tracks)):
            if track_id in miss_counts:
                if not assigned:
                    miss_counts[track_id] += 1  # Increment miss count if not assigned

                # Get current state of the track
                current_state = state_map.get(track_id)

                # Determine miss threshold based on current state
                if current_state and current_state.startswith('Poss'):
                    miss_threshold = 1
                elif current_state and current_state.startswith('Tentative'):
                    miss_threshold = 2
        ...
        try:
            ...
            for track in tracks:
                if track:
                    ...
                    output += f"  Hits: {hit_counts.get(track['track_id'], 0)}, Misses: {miss_counts.get(track['track_id'], 0)}\n"
